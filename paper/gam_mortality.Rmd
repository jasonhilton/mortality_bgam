...
s_{\gamma}(\text{C}) = 0 \;,
\end{aligned}
$$ {#eq:constraints} with C here indicating the most recent cohort. These constraints ensure that linear improvements in mortality with time are estimated as part of the $s_{\beta}(x)$ term. 
...
\pmb{\eta} &\sim \text{Multivarate Normal}(\pmb{0}, ZZ^{T}\sigma_k^{2}).
\end{aligned}
$$ {#eq:eta}
...
\end{bmatrix},
\end{aligned}
$$ {#eq:conditional} where subscripts on the covariance matrices indicate partitions so that $\Sigma_{*\dagger}$ is the sub-matrix of $\Sigma$ with rows corresponding to $\eta^{*}$ and columns to $\eta^{\dagger}$.  Forecast innovations of the period coefficients are unconstrained and so have independent normal distributions with variance $\sigma_k^{2}$.
...
The LOOIC score for males and females for the models with cut-points $k=[80,81,...,95]$ is given below in Figure \ref{fig:looic}. Later cut-points tend to be preferred because the greater flexibility of the GAM model gives higher LOOIC values even at relatively high ages, although the absolute differences between the models are small. Models with points of transition above age 95 are not considered, as this would leave too few data-points with which to estimate the old-age model effectively.
...
```{r looic,echo=FALSE,message=FALSE,error=FALSE,fig.height=3,fig.width=6, warning=FALSE, plot_lab=TRUE, fig.cap="LOOIC values for models using different cut-points"}
...
\end{aligned}
$$ {#eq:stacking}
[@Yao2017, p.7], where $p(y_i | y_{-i}, M_k)$ is approximated using the LOOIC measure described above. The form of the combined predictive distribution is then $\hat{p}(\tilde{y}|y) = \sum_{k=1}^{K}w_k p(\tilde{y}|y, M_k)$. The estimated model weights are shown in Figure @fig:weights; the greatest individual weight is given to models with the latest points of transitions, reflecting the pattern in the LOOIC measure. Other models with earlier cut-points are also given weight, however, reflecting that they perform well at predicting some data-points which are not so well estimated by the late-transition model.
...
\end{aligned}
$$, with the $\epsilon_kf$ and $\epsilon_kf$ referring to period innovations for males and females respectively. The period innovations for each sex retain separate variance parameters, but are assumed to be joint multivariate normal, with the correlation determined by parameter $\rho$. This approach means that joint forecasts can be generated accounting for the fact that in potential futures where mortality for females is high, it will tend to be high for males as well. The prior distribution of the period parameters conditional on the constraints is obtained in a similar way to described in the Section \ref{sec:model}. More detail is given in the appendix.
