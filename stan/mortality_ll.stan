...
transformed parameters {
  ...
  for (i in 1:cutoff){
    ...
    // neg_binomial_2_log_lpmf returns are real so no vectorisation
    for (t in 1:n_years){
      log_lik[i,t] = neg_binomial_2_log_lpmf(deaths[i,t] | lambda_v[t], dispersion);
    }
  }
  ...
  for (i in (cutoff + 1):N){
    for (t in 1:(n_years)){
      if (expos[i, t] != 0 && N - i + t >= first_cohort){
        log_lik[i, t]  = neg_binomial_2_log_lpmf(deaths[i, t] |
                                                 log_expos[i, t] +
